...
$filename = $table."_tableau.".$extension;
...
fwrite_encode( $fp,$insertion_deb_suivi_user."\n" );
...
fwrite_encode( $fp,'<tr>'."\n");
foreach ($tableau_ordre as $key => $val) {
	if (!empty($val)){
		fwrite_encode( $fp,'<th>'."\n" );
...
fwrite_encode( $fp,$insertion_boucle_suivi_user."\n" );
...
foreach ($tableau_ordre as $key => $val) {
	if (!empty($val)){
		...
		if (!empty($fk)){
			//gestion de l'affichage des popup avec 
                        fwrite_encode($fp, "<?php\n");
...
//inclusion de code specifique
fwrite_encode( $fp,$insertion_fin_suivi_user."\n" );
