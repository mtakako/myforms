...
$filename = $table."_view.".$extension;
...
fwrite_encode( $fp,$insertion_deb_view."\n" );
...
while ($i < $fields) {
	...
	if (isset($vue_admin)&&in_array($i,$vue_admin)){
	  fwrite_encode( $fp,'<?php'."\n");
	  fwrite_encode( $fp,'if (($view_admin_zone_vide=="invisible")&&(empty($row['.$i.'])||($row['.$i.']=="0000-00-00")))'."\n");
	  fwrite_encode( $fp,'echo "<tr style=display:none>";'."\n");
	  fwrite_encode( $fp,'else'."\n");
	  fwrite_encode( $fp,'echo "<tr>";'."\n");
	  fwrite_encode( $fp,'?>'."\n");
		if (isset($label)&&(!empty($label)))
                        fwrite_encode( $fp,'<td '.$div_attributs_view[$i].'><?php echo $label['.$i.']?></td>'."\n" );

		else
                        fwrite_encode( $fp,'<td '.$div_attributs_view[$i].'>'.ucfirst(mb_strtolower($name))."</td>\n" );
		...
		elseif (isset($fichiers)&&in_array($i,$fichiers)){
			...
			fwrite_encode($fp,'if (!empty($row['.$i.'])){'."\n");
			fwrite_encode($fp,'   $fichier=$rep.\'/'.$table.'-\'.$row[0].\'-\'.\''.$name.'\'."-".$row['.$i.'];'."\n");
			fwrite_encode($fp,'   echo "$row['.$i.']&nbsp;<a target=_self target=_blank href=\"$fichier\">Voir</a><br />";'."\n");
			fwrite_encode($fp,"}\n");
			fwrite_encode($fp,"?>\n");
			fwrite_encode($fp,"</td>\n");
		}
...
fwrite_encode( $fp,'?>'."\n" );
//inclusion de code specifique
fwrite_encode( $fp,$insertion_fin_view."\n" );
?>
</body>
</html>
